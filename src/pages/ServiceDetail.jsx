...
export default function ServiceDetail() {
  ...
  if (isLoading) {
    return <div className="pt-40 pb-24 text-center text-slate-sub min-h-screen bg-ink">Loading service details...</div>;
  }

  if (!service) {
    return (
      <div className="pt-40 pb-24 text-center min-h-screen bg-ink px-6">
        <h1 className="text-3xl font-bold text-pearl mb-4">Service Not Found</h1>
        <p className="text-slate-sub mb-8">This service page doesn't exist yet.</p>
  ...
  return (
    <div>
      ...
      <section className="pt-8 pb-20 px-6 bg-ink">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <ScrollReveal>
            <div className="w-14 h-14 rounded-2xl bg-indigo-accent/10 flex items-center justify-center mb-6">
            ...
            <p className="mt-6 text-lg text-slate-sub leading-relaxed">{service.heroSubtext || service.shortDescription}</p>
            {service.metricLabel && <p className="mt-4 text-sm font-semibold text-emerald-accent">✓ {service.metricLabel}</p>}
            <div className="mt-8">
              <CTAButton to="/contact" variant="primary">Request Proposal</CTAButton>
            </div>
          </ScrollReveal>
          {service.heroImage && (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {service.deliverables.map((d, i) => (
                <ScrollReveal key={i} delay={i * 0.05} className="flex items-start gap-3 glass-cell rounded-xl p-5">
                  ...
                  <span className="text-sm text-pearl/90">{d}</span>
                </ScrollReveal>
              ))}
            </div>
          </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {service.process.map((p, i) => (
                <ScrollReveal key={i} delay={i * 0.06} className="glass-cell rounded-2xl p-6">
                  <h3 className="text-base font-bold text-pearl mb-2">{p.step}</h3>
                  <p className="text-sm text-slate-sub leading-relaxed">{p.description}</p>
                </ScrollReveal>
              ))}
            </div>
          </div>
